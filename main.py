...
async def extract_text(file: UploadFile = File(...)):
    ...
    try:
        ...
        - 좌측 'DEFECT DESCRIPTION' 칸에 쓰인 실제 결함 내용만 추출하세요.
        - 🚨 단, 문장 맨 앞에 적힌 'ITEM 1', 'ITEM 2', 'A', 'B', 'C' 등의 아이템 번호나 알파벳 기호는 절대 포함하지 말고 과감히 잘라내세요. (예: "ITEM 1 4G SEAT RECLINE FUNCTION..." 이라고 적혀있다면 "4G SEAT RECLINE FUNCTION..." 만 추출해야 합니다.)
        - 절대 우측 'ACTION TAKEN' 칸을 섞지 마세요.

        [6. 적용근거(reason) 분류 🚨 시각 착시(OCR 토큰 오류) 원천 차단 규칙 🚨]
        - 🚨 [가장 중요] ACTION TAKEN 칸의 MEL, NEF, AMM 박스 중 **어느 곳에도 체크(X, V) 표시가 아예 없다면**, 절대 임의로 "MEL" 등을 유추해서 적지 마세요! 이 경우 무조건 빈 문자열("")을 출력해야 합니다.
        - (기존의 판단 공식은 체크가 있을 때만 완벽히 적용합니다)
...
async def smart_search(req: SmartSearchRequest):
    ...
    try:
        ...
        3. 결함 내용에 '31K', '24A' 같은 좌석 번호가 있더라도, 실제 불량난 부품(예: Monitor, Screen, Light, Tray table 등)이 명시되어 있다면 좌석(Seat) 관련 코드보다 해당 부품 관련 코드를 최우선으로 찾아야 합니다.
