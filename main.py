...
async def extract_text(file: UploadFile = File(...)):
    ...
    try:
        ...
        [1. 🚨 정보 창조 금지 & 제한적 항공 용어 유추 허용 🚨]
        - 문서에 아예 없는 내용(없는 결함, 없는 숫자)을 지어내는 것은 절대 금지합니다.
        - 💡 [핵심 변경]: 단, 정비사의 악필로 인해 'DEFECT DESCRIPTION (결함 내용)'의 영어 스펠링이 뭉개졌거나 이상하게 보일 경우, 기계적으로 이상한 글자를 그대로 적지 말고 **항공 정비 전문 용어(Aviation Maintenance Terminology), 부품명, 약어 문맥에 맞게 올바른 스펠링으로 교정(유추)**하여 출력하세요. (예: 'HYD PUMP'가 'H4D POMP'처럼 뭉개져 보여도 항공 문맥에 맞게 'HYD PUMP'로 지능적으로 교정)
        - ATA CODE나 DEFER No. 같은 '숫자/코드' 란이 완전히 비어있다면 무조건 빈 문자열("")을 출력하세요.
