...
async def extract_text(file: UploadFile = File(...)):
    ...
    try:
        ...
        🚨 [초강력 경고: 개별 항목 독립 평가] 🚨
        - 하나의 문서 안에 이월(Deferred) 항목과 종결(Completed) 항목이 위아래로 섞여 있을 수 있습니다.
        - 윗줄에 있는 결함이 이월(Deferred) 대상이라고 해서, 그 아랫줄에 있는 조치 완료된 종결 결함들까지 덩달아 추출해서는 절대 안 됩니다.
        - 반드시 **각 결함(Row)별로 철저하게 독립적으로 판단**하여, 조치가 완료되고 DEFER No.가 없는 '종결 결함'은 추출 결과(items 배열)에서 완벽하게 제외(Skip)하세요. 오직 이월 대상이거나 조치되지 않은 결함만 배열에 담아야 합니다.

        [5. 결함 본문(defect) 추출 및 복원]
        - 아래쪽 넓은 'DEFECT DESCRIPTION' 칸의 내용만 추출하되, 문단 첫 글자(L/H, LU, UD 등) 절대 누락 금지.
        - 🚨 [좌석 번호 및 수직선(1) 정밀 판독 절대 규칙]: 문장 내 **위치를 불문하고 (맨 앞, 중간, 끝 어디든)** 좌석 번호(예: 71K, 11H, 24A 등 '숫자+알파벳' 조합)나 숫자가 등장할 때, 얇은 수직선(|)이나 소문자 'l' 형태의 숫자 '1'을 단순 노이즈나 인접한 알파벳의 획으로 착각하여 누락(예: 71K를 7K로, 11A를 1A로 오독)하는 일이 없도록 문맥을 파악하여 완벽히 복원하세요!
        ...
        ▶ [CABIN LOG 인 경우]
        1. 🚨 NEF 강제 확정 (MEL 오독 절대 금지): 'DEFER No.' 칸에 적힌 번호가 **'XX-XX-XX' (예: 23-30-12 등 오직 6자리 숫자와 대시 기호)** 형태라면 ➡️ 100% "NEF"입니다. 이 경우 "MEL"로 잘못 출력하는 것을 엄격히 금지합니다! 그리고 AMM 으로도 출력하면 안됩니다. AMM 에 옆에 붙어 있다고 AMM 이 아닙니다. 꼭 양식을 보고 NEF 로 판단하길바랍니다. 계속 오판이 많이 일어나고 있습니다. 첫번째 항목은 잘 되는데,두세번째 아이템으로 가면 NEF 항목을 MEL 로 출력하는 경향이 있습니다. 'XX-XX-XX' 양식은 1000000% NEF 이므로 절대로 MEL로 출력하지 마시길 바랍니다.
        2. MEL 강제 확정: 'DEFER No.' 칸 번호 마지막이 문자로 끝나는 'XX-XX-XXY' 형태라면 ➡️ 무조건 "MEL" 추출. (여기서 Y는 A,B,C,D 등 알파벳 1자로 이루어진 문자임.) -> 숫자 +문자로 이루어진 근거를 NEF로 판단하는 것을 엄격히 금지합니다.
        3. AMM 강제 확정: 'DEFER No.' 칸 번호가 12글자 이상이라면 ➡️ 무조건 "AMM" 추출.
        4. 예외사항 : 24-00-11A, 24-00-11B, 24-00-11C 이 세항목은 뒤에 문자가 있더라도 무조건 NEF 로 추출해야함. (예회사항임)
        ...
        [7. ATA CODE 추출 규칙 🚨 억지 유추 금지 및 미세 수정 정밀 판독 🚨]
        - 오직 실제 문서의 'ATA CODE' 란에 수기로 적힌 4자리 숫자만 추출하세요.
        - 💡 [취소선 및 글자 단위 미세 수정 판독]: 만약 숫자 일부에 취소선이 그어지고 그 '위'나 '아래'에 다른 숫자가 조그맣게 적혀 있다면(예: 끝자리 6을 지우고 위에 1을 적음) ➡️ **지워진 숫자는 가차 없이 버리고, 새로 적힌 숫자를 조합하여 정비사가 최종 의도한 4자리 숫자(예: 4421)를 완벽하게 복원하여 추출**하세요!
        - 주변에 'correction' 같은 영단어가 있어도 무시하고 오직 숫자만 정제해서 4자리로 만드세요.
        - 🚨 [초강력 경고]: '적용근거(DEFER No.)' 칸의 번호를 보고 앞 4자리를 잘라내어 ATA에 채워 넣는 "오지랖(유추 행위)"을 절대 금지합니다!
        - 문서의 ATA 칸이 진짜로 비어있을 때만 빈 문자열("")을 출력하세요.
        ...
        [10. 🚨 필기체 시각 오독(OCR 오류) 및 근접성 편향 절대 방어 🚨]
        - [기번 창조(Hallucination) 절대 금지]: 'AIRCRAFT REG. NO.'를 판독할 때, 눈에 보이지도 않는 엉뚱한 번호(예: 사진엔 7740인데 8071로 출력)를 무작위로 지어내는 행위를 엄격히 금지합니다. 제공된 DB 목록을 먼저 의식하지 말고, 눈앞의 수기 숫자 획을 있는 그대로 정직하게 판독하세요.
        - [체크박스 절대 우선 (거리 착각 방지)]: 'DEFER No.' 칸에서 수기 번호(예: 25-30-05)가 인쇄된 'AMM ▢' 글자 바로 옆에 바짝 붙어 적혀 있더라도, 절대 거리에 현혹되지 마세요! 정비사가 직접 펜으로 표시한 'V(체크)' 자국(예: NEF ✔️)이 문서의 종류를 결정하는 1순위 절대 증거입니다.
        - [숫자 오독 강제 주의]: 항공기 정비에서 '05'의 '0' 꼬리가 펜 떼짐으로 약간 뻗쳐 있어도 완벽한 '6'이 아니라면 '0'으로 읽어야 합니다. 필기체의 미세한 삐침에 속아 '05'를 '65'로 오독하지 않도록 문맥과 형태를 이중으로 검증하세요.
