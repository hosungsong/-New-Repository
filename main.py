...
async def extract_text(file: UploadFile = File(...)):
    ...
    try:
        ...
        # [수정할 부위 핀포인트 교체]
        [4. 🚨 이월(DEFER) 항목 추출 조건 (가장 엄격한 1순위 절대 규칙) 🚨]
        - 이월(DEFER) 항목이 아닌 행은 절대로 추출(출력)하지 마세요. 불필요한 빈 항목이 하나라도 포함되면 분석이 실패합니다.
        - 🚨 [추출 절대 금지]: 결함 내용(DEFECT) 칸에 아무리 많은 글씨가 적혀 있어도, 그 칸에 대응하는 체크박스(네모칸)에 펜 자국(체크)이 없고, 적용근거 번호(MEL/NEF 등)도 전혀 안 적혀 있다면 ➡️ **무조건 무시하고 절대로 items 배열에 넣지 마세요.**
        - "사진이 잘려서 안 보일지도 모른다"는 핑계로 추출하지 마세요. 명확한 체크나 번호가 눈에 보이지 않으면 무조건 버리세요.
        ...
        ▶ [CABIN LOG 인 경우]
        1. 시각 추출: 체크박스(네모칸)에 펜 자국이 있다면, 그 '바로 왼쪽'에 인쇄된 단어를 1차 후보로 추출합니다.
        2. NEF 교차검증: 수기로 적힌 번호가 'XX-XX-XX' (숫자로만 이루어진 6글자 포맷)이고, 1차 후보가 'NEF'라면 ➡️ 무조건 "NEF" 추출.
        3. MEL 교차검증: 1, 2번과 상이하더라도, 수기로 적힌 번호의 마지막이 문자로 끝나는 'XX-XX-XXA' (숫자+문자 조합) 형태라면 ➡️ 무조건 "MEL" 추출.
        4. AMM 교차검증: 위 규칙들과 상이하더라도, 수기로 적힌 번호가 'XX-XX-XX-XX-XXX-XXXA' 등 12글자 이상 길게 써 있다면 ➡️ 무조건 "AMM" 추출.
        
        ▶ [FLIGHT & MAINTENANCE LOG 인 경우]
        1. 시각 추출: 체크박스(네모칸)에 펜 자국이 있다면, 그 '바로 위'에 인쇄된 단어를 1차 후보로 추출합니다.
        2. MEL 교차검증: 1차 후보가 'MEL'이고, 수기 번호가 'XX-XX-XXA'와 같이 마지막이 문자로 끝나는 형태라면 ➡️ 무조건 "MEL" 추출.
        3. NEF 교차검증: 1, 2번과 상이하더라도, 수기 번호가 'XX-XX-XX' (숫자로만 6글자) 형태이고, 1차 후보가 'NEF'라면 ➡️ 무조건 "NEF" 추출.
        4. AMM 교차검증: 위 규칙들과 상이하더라도, 수기 번호가 12글자 이상(예: XX-XX-XX-XX-XXX-XXXA)이라면 ➡️ 무조건 "AMM" 추출.
        5. CDL 교차검증: 2번, 3번에서 걸러지지 않았고, 수기 번호가 'XX-XX' (2글자씩 2묶음, 총 4숫자) 형태라면 ➡️ 무조건 "CDL" 추출.
        6. SRM 교차검증: 위 1~5번에 해당하지 않으면 SRM일 가능성이 큽니다. 1번 시각적 체크 위치를 다시 확인하여 SRM 칸에 체크되어 있다면 ➡️ "SRM" 추출.
        7. 공란 유지: 위와 같이 모든 규칙을 검증했는데도 확실하지 않거나 모순된다면, 임의로 지어내지 말고 빈 문자열("")을 유지하세요.

        💡 [출력 포맷 공통 규칙]: 확정된 문서 종류와 번호를 띄어쓰기로 합쳐서 하나로 출력 (예: "NEF 25-30-08", "MEL 25-21-02A"). 번호 뒤의 'CAT C', 'CAT B' 등급 표시는 완전히 자를 것.

        [7. ATA CODE 추출 규칙]
        - 오직 숫자 4자리만 골라내어 추출하세요. (예: 44-20 ➡️ 4420)
